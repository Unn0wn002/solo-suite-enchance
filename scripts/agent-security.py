#!/usr/bin/env python3
"""Read-only security checks for the local Solo Suite agent platform."""
...
def npm_audit() -> tuple[str, int, dict[str, object] | None]:
    ...
    try:
        ...
        evidence: dict[str, object] | None = {
            ...
            "temporary_high_advisory_exceptions": [],
...
def main() -> int:
    ...
    parser.add_argument("--full-scanners", action="store_true", help="Run the pinned project-local scanner suite.")
    parser.add_argument("--write-evidence", action="store_true", help="Write normalized npm-audit evidence; requires --npm-audit.")
    ...
    if args.npm_audit:
        ...
        if args.write_evidence and evidence is not None:
            ...
            NPM_AUDIT_EVIDENCE.write_text(json.dumps(evidence, indent=2) + "\n", encoding="utf-8", newline="\n")
    ...
    for scanner in ["semgrep", "gitleaks", "trivy", "pip-audit"]:
