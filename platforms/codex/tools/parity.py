...
# Reviewed Codex overlays may intentionally merge a command body into an
# existing specialist or replace a Claude workflow with a native wrapper.
...
BODY_ADAPTER_WAIVERS = {
    ...
    "solo:solo-full-team-dev": "authoritative Codex AgentRoom compatibility wrapper",
    "seo:seo": "Codex SEO specialist with merged command routing and output contract",
    **{
        f"seo:{skill}": "Codex SEO specialist with merged command routing and output contract"
        for skill in (
            "seo-backlinks", "seo-cluster", "seo-competitor-pages",
            "seo-content", "seo-content-brief", "seo-drift", "seo-ecommerce",
            "seo-flow", "seo-geo", "seo-google", "seo-hreflang", "seo-images",
            "seo-local", "seo-maps", "seo-page", "seo-plan", "seo-programmatic",
            "seo-schema", "seo-sitemap", "seo-sxo", "seo-technical",
        )
    },
}
...
def _transform_command_body(body: str, dependency_skill_names: Iterable[str]) -> str:
    ...
    for name in dependency_skill_names:
        ...
        transformed = re.sub(
            rf"Use the {escaped} orchestrator skill",
            lambda _match, value=replacement: value,
            transformed,
            flags=re.IGNORECASE,
        )
...
def source_plugins(source: Path) -> list[tuple[str, Path]]:
    ...
    if not result:
        raise ParityError("source checkout has no plugins")
...
def source_commands(source: Path) -> list[dict[str, object]]:
    ...
    for plugin in sorted(path.name for path in (source / "plugins").iterdir() if path.is_dir()):
        ...
        if not command_root.is_dir():
            continue
        # Match the converter's legacy-invocation ordering by comparing the
        # command stem, not the filename including the `.md` suffix.
        for path in sorted(command_root.iterdir(), key=lambda item: item.stem):
    ...
    if not commands:
        raise ParityError("source checkout has no command workflows")
...
def source_specialists(source: Path) -> list[dict[str, object]]:
    ...
    for path in sorted((source / "plugins").glob("*/skills/*/SKILL.md")):
        ...
        result.append(item)
    if not result:
        raise ParityError("source checkout has no specialist skills")
...
def build_manifest(source: Path) -> dict[str, object]:
    ...
    command_skill_ids = {
        f"{item['plugin']}:{item['skill_name']}" for item in commands
    }
    specialist_skill_ids = {str(item["id"]) for item in specialists}
    allowed_extra_ids = set(ALLOWED_EXTRA_SKILLS)
    available_body_waivers = command_skill_ids | specialist_skill_ids
    # The published Claude v1.0.27 source predates the later SEO adapter
    # overlay and its historical manifest did not declare body waivers.  The
    # current working tree does contain that overlay, so keep its waivers in
    # the current manifest while reproducing the historical manifest exactly
    # for the 18-plugin source archive.
    include_body_waivers = any(name == "seo" for name, _manifest in plugins)
    ...
    if include_body_waivers:
        adapter["body_waivers"] = [
            {"id": key, "reason": BODY_ADAPTER_WAIVERS[key]}
            for key in sorted(BODY_ADAPTER_WAIVERS)
            if key in available_body_waivers
        ]
    ...
    return {
        ...
        "counts": {
            ...
            "source_specialist_skills": len(specialists),
            "target_skills": len(
                command_skill_ids | specialist_skill_ids | allowed_extra_ids
            ),
        },
...
def check_target(source: Path, target: Path, manifest: dict[str, object]) -> list[str]:
    ...
    body_waivers = {
        str(item["id"])
        for item in adapter.get("body_waivers", [])
        if isinstance(item, dict) and isinstance(item.get("id"), str)
    } if isinstance(adapter, dict) else set()
    ...
    if actual_map != expected_map:
        _record_error(errors, "command-map.json does not exactly match the canonical 125-command map")
    ...
    for item in specialists:
        ...
        if skill_id not in WAIVER_SKILLS and skill_id not in body_waivers:
            if not target_path.is_file():
    ...
    # Exactly the canonical 183 skills plus the one declared Codex extra.
    ...
    expected_target_count = int(expected["counts"]["target_skills"])
    if len(actual_skill_ids) != expected_target_count:
        _record_error(errors, f"expected {expected_target_count} target skills, found {len(actual_skill_ids)}")
    ...
    if len(policy_paths) != expected_target_count:
        _record_error(errors, f"expected {expected_target_count} openai.yaml policies, found {len(policy_paths)}")
...
def generate(source: Path) -> Path:
    ...
    print(
        f"Generated {destination} ({value['counts']['plugins']} plugins, "
        f"{value['counts']['source_commands']} commands, "
        f"{value['counts']['source_specialist_skills']} specialist skills)"
    )
    return destination
...
def check(source: Path, target: Path) -> int:
    ...
    print(
        ...
        f"{expected['counts']['plugins']} plugins, "
        f"{expected['counts']['source_commands']} commands, "
        f"{expected['counts']['source_specialist_skills']} specialist skills, "
        f"{expected['counts']['target_skills']} target skills, "
