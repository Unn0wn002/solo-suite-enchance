...
# Literal drift guards, mirroring tests/test_inventory.py: adding or removing a
# plugin/command/skill must be a deliberate edit here too. Source counts are the
# Claude checkout; the target count is the Codex adapter's skill inventory
...
def write_json(path: Path, value: object) -> None:
    ...
    # NOTE: Path.write_text() only grew a `newline` parameter in Python 3.10,
    # and this repository supports 3.9 (see the CI matrix). io.open takes it on
    # every supported version, and pinning "\n" keeps the generated contract
    # byte-identical on Windows checkouts.
...
def check_target(source: Path, target: Path, manifest: dict[str, object]) -> list[str]:
    ...
    if actual_map != expected_map:
        _record_error(
            errors,
            "command-map.json does not exactly match the canonical "
            f"{len(expected_map)}-command map",
        )
    ...
    # Exactly the canonical skill inventory plus the declared Codex extras.
