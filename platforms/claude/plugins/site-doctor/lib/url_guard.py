#!/usr/bin/env python3
"""url_guard.py — shared SSRF guard for bundled helper scripts. Stdlib only.

CANONICAL SOURCE: plugins/site-doctor/lib/url_guard.py
MIRRORED COPY:    plugins/seo/lib/url_guard.py

The two files must stay BYTE-IDENTICAL. Each plugin ships its own copy because
plugins install independently -- ``seo`` must work when ``site-doctor`` is not
installed, and ``${CLAUDE_PLUGIN_ROOT}`` resolves per plugin, so a cross-plugin
import would break the standalone install. Edit the canonical file, then mirror
it verbatim. ``tests/test_url_guard.py`` and ``self_check.py`` both fail closed
if the copies ever diverge, so a security fix cannot land in only one of them.
...
# ---- origin binding -------------------------------------------------------
# After safe_get() has followed and re-validated every redirect hop, the caller
# still has to decide whether the FINAL url may be treated as evidence for the
# url that was requested. That decision used to be copy-pasted into individual
# helper scripts, where it could drift apart silently; it lives here now, next
# to the policy it complements.


def normalized_hostname(value):
    """Return a structural lower-case hostname for a URL or bare netloc.

    Returns None when *value* has no parseable host, so callers must treat
    None as "cannot be compared" rather than as a match.
    """
    try:
...
def same_site_url(left, right):
    ...
    Deliberately NOT a registrable-domain (eTLD+1) comparison: a sibling
    subdomain is a different origin for audit purposes.
    """
...
def same_audit_origin(left, right, allow_http_upgrade=False):
    ...
    With *allow_http_upgrade* the single permitted scheme change is the
    ordinary http:80 -> https:443 redirect; every other cross-scheme or
    cross-port pair is refused.
    """
    try:
