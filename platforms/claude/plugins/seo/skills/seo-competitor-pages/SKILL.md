...
If a DataForSEO-style or similar SERP/keyword connector is available, use it for real search volume, difficulty, and current SERP composition on the target comparison keywords instead of estimating. Without one, fetch competitor pages through `${CLAUDE_PLUGIN_ROOT}/lib/url_guard.py`-guarded requests for the feature/pricing data, mark volume/difficulty estimates as qualitative, and say so.
