...
If a DataForSEO-, Ahrefs-, or similar SERP/keyword connector is available, use it for real SERP composition, keyword volume, difficulty scoring, intent classification, and competitor content parsing instead of qualitative estimates — say so explicitly. Without one, fetch the target and competitor pages through `${CLAUDE_PLUGIN_ROOT}/lib/url_guard.py`-guarded requests, keep volume/difficulty qualitative, and say so.
