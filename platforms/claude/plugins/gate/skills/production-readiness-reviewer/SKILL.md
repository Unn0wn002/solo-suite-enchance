...
description: Score whether an app is ready for real users across 14 categories - product, architecture, design, frontend, backend, database, security, testing, performance, SEO, analytics, deployment, monitoring, documentation. Use ONLY when the user wants a NUMBER or an overall launch verdict - "score my readiness", "how ready are we", launch readiness, go-live readiness. Do NOT use for a yes/no checkpoint at a specific moment (use quality-gatekeeper), for producing the deploy, rollback or CI artifacts (use devops-engineer), or for auditing a live site's technical health (use website-audit). Produces per-category scores (each /10; N/A categories accepted by the applicability matrix leave the denominator, so normalized = round(total / (10 x applicable) x 100)) and a launch status - BLOCKED / SAFE WITH WARNINGS / SAFE TO LAUNCH - where any critical failure forces BLOCKED.
---
