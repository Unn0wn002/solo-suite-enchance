---
description: Score launch readiness /100 and give a launch verdict
argument-hint: [optional scope, environment (default production)]
---

> **Running helpers:** `${CLAUDE_PLUGIN_ROOT}` is set by Claude Code to this plugin's installed root, so every command below works from any working directory. If `python3` is not on PATH, use `python` (macOS/Linux/Windows) or `py -3` (Windows launcher) instead. On Windows a bare `python3` may resolve to the Microsoft Store App Execution Alias, which is an installer stub, not an interpreter — it prints "Python was not found" and exits 9009, and `gate_policy` refuses it.
