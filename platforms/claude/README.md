...
For the repository-intelligence core used by the `repo` workflows, follow the
root [`CORE_REPOSITORY_INTELLIGENCE.md`](../../CORE_REPOSITORY_INTELLIGENCE.md).
...
Do not run Graphify's Claude platform installer from repo setup because it can
create an automatic hook. Claude uses the repository's generated skill mirror.
