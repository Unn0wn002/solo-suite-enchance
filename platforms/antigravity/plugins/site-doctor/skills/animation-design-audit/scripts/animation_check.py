...
project root for:
  ...
  - cleanup signals: gsap.context/.revert(), useGSAP, ScrollTrigger.kill(),
    .kill() on stored tween/timeline refs, or gsap.killTweensOf() — vs.
    tween/timeline creation with none of these anywhere in the same file
  - prefers-reduced-motion handling in JavaScript/TypeScript/component code
    or CSS-family stylesheets anywhere in the scanned tree
...
def main():
    ...
    for path in iter_source_files(root):
        ...
        has_gsap = (
            bool(GSAP_IMPORT_RE.search(text))
            or bool(GSAP_CDN_RE.search(text))
            or bool(TWEEN_CALL_RE.search(text))
        )
        if not has_gsap:
            continue
    ...
    if cdn_hits and gsap_in_package:
        ...
        print(f"\n[WARN] GSAP is loaded via both a CDN <script> tag AND the "
              f"npm package in {len(cdn_hits)} file(s) — e.g. {json.dumps(cdn_hits[0])}. "
    ...
    if files_with_layout_anim:
        ...
        print(f"\n[WARN] {len(files_with_layout_anim)} file(s) appear to "
              ...
              f"left/margin/padding) directly, e.g. {json.dumps(files_with_layout_anim[0])}. "
    ...
    if files_with_tweens_no_cleanup:
        ...
        print(f"\n[FAIL] {len(files_with_tweens_no_cleanup)} file(s) create "
              ...
              f"e.g. {json.dumps(files_with_tweens_no_cleanup[0])}. In a component that "
    ...
    if scrolltrigger_files_no_refresh_awareness:
        ...
        print(f"\n[WARN] {len(scrolltrigger_files_no_refresh_awareness)} "
              ...
              f"nearby, e.g. {json.dumps(scrolltrigger_files_no_refresh_awareness[0])}. "
