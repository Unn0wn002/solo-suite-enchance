...
def main(argv=None):
    ...
    if not os.path.exists(args.root):
        print("Path not found: %s" % json.dumps(args.root))
        return 2
    root = os.path.abspath(args.root)
    if not (os.path.isdir(root) or os.path.isfile(root)):
        print("Root must be a directory or a regular file: %s"
    ...
    for h in hits:
        ...
        print("    %s:%d" % (json.dumps(h["path"]), h["line"]))
        print("    preview: %s" % json.dumps(h["preview"]))
        print("    %s\n" % h["fingerprint"])
    if history_hits:
