...
description: Decide GO or NO-GO at one specific checkpoint - before coding, before merge, or before deploy. Use ONLY when the user is standing at one of those three moments - gate, "ready to merge", "ready to deploy", "can I ship this", "can I start coding". Do NOT use when the user wants a readiness SCORE or an overall launch verdict (use production-readiness-reviewer), when they want the deploy or rollback plan itself (use devops-engineer), or when they want a technical audit of a running site (use website-audit). Returns a verdict plus the exact blockers; hard-blocks on critical gaps rather than averaging them away.
---
