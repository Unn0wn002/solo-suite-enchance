...
def resolve_executable(tok, root):
    ...
    if _inside(resolved, root):
        return None, ("executable %r resolves INSIDE the project root "
                      ...
                      (tok, resolved))
    if _is_windows_store_alias(resolved):
        return None, ("%r resolves to the Windows App Execution Alias %r, "
                      "which is a Microsoft Store installer stub, not a "
                      "Python interpreter — it prints \"Python was not "
                      "found\" and exits 9009. Use `python` or `py -3`, or "
                      "disable the alias in Settings > Apps > Advanced app "
                      "settings > App execution aliases" % (tok, resolved))
    return resolved, None


def _is_windows_store_alias(resolved):
    """True for the zero-byte WindowsApps reparse stubs.

    `shutil.which("python3")` SUCCEEDS on a stock Windows 11 box because
    %LOCALAPPDATA%\\Microsoft\\WindowsApps\\python3.exe exists, so a
    which()-style guard passes and the failure only shows up at run time as
    exit 9009. The stubs are zero-length reparse points, so both the path
    location and the size are checked; either alone would be brittle.
    """
    if os.name != "nt":
        return False
    lowered = resolved.replace("/", "\\").lower()
    if "\\microsoft\\windowsapps\\" not in lowered:
        return False
    try:
        return os.path.getsize(resolved) == 0
    except OSError:
        # An unreadable reparse point in WindowsApps is not a usable
        # interpreter either.
        return True
