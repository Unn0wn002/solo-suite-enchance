...
description: Act as the DevOps engineer for a solo developer - produce the release ARTIFACTS - a pre-release preflight checklist, a safe deployment plan, a rollback plan prepared before shipping, and the CI pipeline that runs the project's checks on every PR. Use ONLY when the user wants one of those documents built - "plan the deploy", "write a rollback plan", "set up CI", "how do I deploy this safely", "what if it breaks". Do NOT use to judge readiness - a score belongs to production-readiness-reviewer, a GO/NO-GO checkpoint to quality-gatekeeper, and a live-site technical audit to website-audit. Reads .solo/ for release context; leans on site-doctor's infra/deploy/backup skills for depth when installed.
---
