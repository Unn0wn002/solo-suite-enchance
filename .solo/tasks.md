...
- [ ] T28 (new, 2026-08-08): **CI's security gate is red on `main` and will stay red** until `vinext` bumps `image-size`. Two HIGH DoS advisories, no upstream fix, assessed not reachable on this deployment (full evidence in `risks.md`). Decide between: (a) leave red and accept that the gate now signals nothing day to day — the alert-fatigue failure mode; (b) add a **narrow, expiring** exception keyed to GHSA-w3rx-r6r6-pgpr and GHSA-5p2g-fcmc-qvqq in `scripts/agent-security.py`, so the gate stays meaningful and re-fires the moment the advisory set changes or the exception lapses; (c) downgrade `vinext` — rejected, the breaking five-version rollback is the larger risk. **Recommended: (b).** A gate that is permanently red is functionally the same as no gate.  (feature: security)
