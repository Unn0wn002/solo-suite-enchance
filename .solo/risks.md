...
| A diagnostic `git stash`/`git stash pop` (run while investigating the token-report staleness, unrelated to security scanning) silently flipped several hash-pinned files from LF to CRLF via this repo's `core.autocrlf=true` setting, breaking their recorded SHA-256 checks — a self-inflicted issue found and fixed in the same session (see `decisions.md` and `tasks.md`'s T24 entry for the full account). Worth knowing for future sessions: **avoid `git stash` in this repo**, or explicitly set `core.autocrlf=false` for the duration if one is truly needed. | `sha256sum`/`file` output before and after the fix, 2026-08-07 | whoever runs git operations here | **Root cause fixed 2026-08-07** — see the row below |
| **Root cause of the row above**: the repository root had **no `.gitattributes`** while every machine here runs `core.autocrlf=true`. Only `platforms/{claude,codex,antigravity}/` carried one. So the LF→CRLF rewrite was never specific to `git stash` — a plain `git checkout`, `git switch`, `git pull`, `git restore`, or fresh clone would have done the same to every hash-pinned text file outside `platforms/`. The "avoid `git stash`" guidance addressed one trigger and left the more common ones open. Additionally, 62 tracked files were found **already CRLF on disk** (`package.json`, `README.md`, `CLAUDE.md`, `Makefile`, `tsconfig.json`, `app/*`, `db/*`, `worker/*`, …) — none of them hash-pinned, so nothing was broken, but it shows the conversion had already been happening routinely. | `git ls-files --eol` (62 × `w/crlf`, 1 × `w/mixed`); root `.gitattributes` confirmed absent, 2026-08-07 | whoever runs git operations here | **Fixed** — root `.gitattributes` added with `* text=auto eol=lf` plus explicit `binary` for assets and `*.sig`, matching the policy `platforms/*/.gitattributes` already used. Verified by checking out the committed tree into a scratch worktree: every pinned file arrives LF and every recorded SHA-256 matches. |
